...
// Reconstruct delegation lineage
// IMPORTANT:
// DO NOT filter revoked edges here.
// Reconstruction and policy validation
// are separate phases.

OPTIONAL MATCH
  chain_path =
    (human:Human)
      -[:DELEGATED_TO]->(root:Agent)
...
WITH
  ...
  [rel IN relationships(chain_path) | {
    ...
    permissions:  coalesce(
      rel.inherited_permissions,
      []
    )
  }]
    AS chain_rels

// Core chain validation
WITH
  agent,
  tool,
  human,
  ...
  [r IN chain_rels | r.scope_id]
    AS scope_chain,

  [r IN chain_rels | r.permissions]
    AS perm_chain,

  reduce(
    maxDepth = 0,
    r IN coalesce(chain_rels, [])
      |
      CASE
        WHEN coalesce(
          r.depth,
...
RETURN
  ...
  $action_type
    IN effective_permissions
      AS action_within_scope,

  scope_correctly_attenuated
    AS scope_correctly_attenuated,
...
> {
  ...
  try {
    ...
    if (!record) {
      ...
      throw new Error(
        "Unexpected empty authorization record",
      );
    }
